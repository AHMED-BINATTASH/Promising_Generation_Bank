...
namespace Promising_Generation_Bank_API.Data.Repositories
{
    public class TransactionRepository
    {
        ...
        public async Task<List<AchievementDto>> GetCompletedAchievementsAsync()
        {
            var achievements = await _context.Transactions
                .AsNoTracking()
                .OrderByDescending(t => t.Date)
                .Select(t => new AchievementDto
                {
                    ChildName = t.Child.Name,
                    QuestName = t.ActivityName,
                    RewardAmount = t.CashAmount,
                    Date = t.Date
                })
                .ToListAsync();
